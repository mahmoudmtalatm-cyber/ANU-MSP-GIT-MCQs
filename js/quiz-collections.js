...
/** Deletes a folder. Its own subfolders move up to become children of ITS
 *  parent (never deleted, never orphaned); any quiz filed directly inside
 *  it becomes Uncategorized. Nothing about a quiz's content is ever
 *  touched — this only ever changes where it's filed. */
async function cqDeleteCollection(id) {
  const collections = loadQuizCollections();
  const col = collections.find(c => c.id === id);
  if (!col) return;
  ...
  const parts = [`Delete "${col.name}"?`];
  if (children.length) parts.push(`Its ${children.length} subfolder${children.length !== 1 ? 's' : ''} will move up to ${col.parentId ? "its parent folder" : "the top level"}.`);
  if (directQuizCount) parts.push(`${directQuizCount} quiz${directQuizCount !== 1 ? 'zes' : ''} directly inside will become Uncategorized — nothing is deleted.`);
  ...
  const { saveQuizCollection, deleteQuizCollectionRaw } = await import('./local-store.js');
