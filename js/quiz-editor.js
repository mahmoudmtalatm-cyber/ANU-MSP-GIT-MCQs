...
/* #98 — Publishes every quiz currently checked in the bulk multi-select
   picker (js/admin-panel.js: adminSelectedQuizIds) into adminPubTargetSubject
   as separate lectures, in one action. Mirrors adminPublishQuiz's per-quiz
   pipeline (image hydration/inlining, id assignment, manifest write) but
   runs it once per selected quiz, sequentially — sequential rather than
   parallel so the shared Gemini-adjacent upload path (putContentItem) never
   has to juggle several large image-inlining payloads at once, and so the
   status line can report clean "N of M" progress. No "edit before
   publishing" step here — that only makes sense for one quiz at a time, and
   stays available via the regular single-quiz picker. No before/after
   insert-position either; bulk-published lectures are simply appended after
   whatever's already in the subject, in the order they were selected. */
async function adminBulkPublishQuizzes() {
  if (adminBusy) return;
  if (!adminPubTargetSubject) return;
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    const nameInput = document.getElementById('adminBulkName_' + key);
    let lectureName = (nameInput?.value || adminBulkNameOverrides[key] || '').trim();
    if (!lectureName) lectureName = meta.title;

    if (statusEl) {
      statusEl.innerHTML = `<div class="cq-status">⏳ Publishing ${i + 1} of ${selected.length}: “${escapeHtml(lectureName)}”…</div>`;
    }

    try {
      let questions = meta.questions;
      if (meta.sourceType === 'community') {
      ...
      _cqNormalizeCaseGroups(cleanQuestions);
      _stripEditorTransientFields(cleanQuestions);

      const lectureId = 'pub_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8) + '_' + i;
      const publishedAt = Date.now();
      // Each successive quiz in the batch gets a slightly larger order value
      // than the last, so they keep the same relative order the admin had
      // them in here, and all land after every already-published lecture.
      const order = baseTime + i;
      ...
      failures.push({ title: lectureName, error: e.message || String(e) });
    }
  }

  let finalStatusHtml = '';
  if (!failures.length) {
    finalStatusHtml = `<div class="cq-status success">✅ Published ${succeeded} lecture${succeeded !== 1 ? 's' : ''} to ${escapeHtml(subjects[targetSubject].label || targetSubject)}!</div>`;
  } else {
    finalStatusHtml = `<div class="cq-status error">
      ⚠️ Published ${succeeded} of ${selected.length}. Failed: ${failures.map(f => `“${escapeHtml(f.title)}” (${escapeHtml(f.error)})`).join(', ')}
    </div>`;
  }
  if (statusEl) statusEl.innerHTML = finalStatusHtml;

  // Clear the selection only for the quizzes that actually succeeded, so a
  // partial failure leaves the failed ones checked and ready to retry
  // instead of forcing the admin to re-pick everything from scratch.
  if (succeeded) {
    const failedTitles = new Set(failures.map(f => f.title));
    selected.forEach(m => {
      const key = m.sourceType + ':' + m.sourceId;
      const finalName = (document.getElementById('adminBulkName_' + key)?.value || m.title).trim();
      if (!failedTitles.has(finalName)) {
        adminSelectedQuizIds.delete(key);
        delete adminBulkNameOverrides[key];
      }
    });
  }
  ...
  const freshStatusEl = document.getElementById('adminBulkStatus');
  if (freshStatusEl) freshStatusEl.innerHTML = finalStatusHtml;
}
