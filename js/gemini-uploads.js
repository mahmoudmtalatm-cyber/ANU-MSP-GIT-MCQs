...
3. SYMBOLS AND SPECIAL CHARACTERS — get these exactly right, this is a common source of errors: reproduce every symbol, Greek letter, subscript, superscript, and special character using the ACTUAL Unicode character, never a spelled-out word, never a plain-English description, and never a similar-looking substitute. This applies everywhere — question stems, answer choices, and case/vignette text alike. In particular:
   - Greek letters, which appear constantly in medical/scientific material (β-blocker, α-adrenergic receptor, γ-glutamyl transferase, δ wave, μg, Δ, θ, λ, σ, ω, π, χ², etc.) MUST be output as the real Greek glyph (β, α, γ, δ, μ, Δ, θ, λ, σ, ω, π, χ) — never spelled out as "beta", "alpha", "gamma", etc., and never swapped for a similar Latin letter (β is not "b"/"B", μ is not "u").
   - Ion/chemical notation must keep its exact charge signs and counts, e.g. Na+, K+, Ca2+, Mg2+, HCO3-, CO2, H2O, O2, ATP — do not drop the + / - / superscript numbers or "simplify" the formula.
   - Comparison, math, and arrow symbols (±, ≥, ≤, ≠, →, ↑, ↓, °, %, ½, ¼) must be kept as that exact symbol — do not translate them into words (e.g. never turn "↑" into "increased", "≥" into "greater than or equal to", or "±" into "plus or minus").
   - Superscripts/subscripts (cm², mm³, T1, T2, CO2, H2O) must be preserved exactly as they visually appear in the source, not flattened into plain digits or reworded.
   - If a symbol is genuinely blurry or ambiguous in the source, use your best judgment to output the actual symbol most likely intended (using context, e.g. surrounding medical terminology) rather than falling back to a word, a guessable placeholder, or omitting it entirely.
4. Reproduce EVERY answer choice EXACTLY as written, in the same order, using the same labels (A, B, C, D, E, ... — convert numeric labels like 1,2,3 to A,B,C). Do not omit, reorder, merge, or reword any choice.
5. DO NOT DUPLICATE THE ANSWER CHOICES INSIDE THE QUESTION TEXT — this is a known failure mode, watch for it carefully: the "question" field must contain ONLY the stem (the scenario/context plus the actual question being asked) and must stop there. Every answer choice belongs in "options" ONLY, exactly once — never also copied or re-listed inside "question" as well. This matters most when the source visually runs the choices straight into the question paragraph with no line break or clear separation (e.g. "...which of the following is the most likely diagnosis? A) Diagnosis one B) Diagnosis two C) Diagnosis three D) Diagnosis four") — split the choices out into "options" as normal, but end "question" right where the actual question stops, BEFORE the first choice label ("A)"/"A."/"(A)"/etc.), and do not re-include any of that choice text a second time inside "question". Before finalizing each question, check whether any option's text also appears verbatim (or near-verbatim) inside the "question" field, and if so remove it from "question", keeping it only in "options". The one exception: keep something in "question" if it's genuinely part of the stem's own narrative (e.g. a lab value, drug name, or finding mentioned while describing the case) that merely happens to resemble an option, rather than being one of the actual listed choices themselves.
6. If the document indicates which choice is correct (circled, bolded, underlined, highlighted, starred, checked, or listed in a separate answer key), use EXACTLY that choice as the "answer" for that question. Do not second-guess or change a marked answer.
7. If a specific question has NO indicated correct answer anywhere in the document, set its "answer" to the special value "__NO_KEY__". Do NOT guess or infer an answer — use "__NO_KEY__" exactly.
8. Do not invent, add, duplicate, or remove any questions or options that are not present in the source document.
9. For each question, set "has_image" to true if THIS SPECIFIC question is accompanied by an image, diagram, figure, table, chart, graph, X-ray, CT scan, ECG, histology slide, or any other visual element that is part of it. Set it to false otherwise.
